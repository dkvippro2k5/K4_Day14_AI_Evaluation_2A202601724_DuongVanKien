...
class LLMJudge:
    ...
    def score_response(
        ...
        try:
            ...
            scores = {k: float(parsed[k]) for k in rubric}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        first_avg = sum(first_entry_scores) / len(first_entry_scores) if first_entry_scores else 0.0
        positional_bias = len(scores_batch) > 1 and all(
            (sum(entry.get("scores", {}).values()) / len(entry.get("scores", {})) if entry.get("scores") else 0.0) <= first_avg
            for entry in scores_batch
        )
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant"):
            suggestions.append(
                "Improve prompt clarity and intent routing to keep answers on-topic"
            )
        if categories.get("incomplete"):
            suggestions.append(
                "Increase chunk size or retrieval top-k to reduce context fragmentation"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Review intent detection and system prompt scope boundaries"
            )
        if categories.get("refusal"):
            suggestions.append(
                "Loosen overly strict guardrails for in-scope questions"
            )

        while len(suggestions) < 3:
            suggestions.append(
                "Add few-shot examples showing complete, grounded answers"
            )
